...
export default function ({
  ...
  class List extends Component<IProps> {
    ...
    pageSizeChange = (cur: number, size: number) => {
      ...
      Store.urlSetForm({ name, url: location.search })
      const valObj = {}
      valObj[dfFormat.page] = 1
      valObj[dfFormat.currentPage] = 1
    ...
    render() {
      const { Store, name = 'list', Auth, itemMap = dfIemMap, UI: { layout: { clientWidth } } = UI } = this.props
      ...
      const { isExport = false, isSearch = true, tabs, fields = [], tabField } = listFormConf
      ...
      const errno = listData[dfFormat.errno]
      const errmsg = listData[dfFormat.errmsg]
